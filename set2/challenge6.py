...
from challenge3 import find_AES_mode, encrypt_AES_ECB
...
def find_appended_prefix(encryption_func, prefix_length, block_size):
    known = b''
    while True:
        ith_char = get_one_byte_prefix(encryption_func, prefix_length,
                                       known, block_size)
        if ith_char == None:
            break
        known += ith_char
...
def get_one_byte_prefix(encryption_func, prefix_length, known, block_size):
    ...
    for input_byte in range(0, 0xff+1):
        ...
        guess_output = get_range(encryption_func(
            padding + known + as_bytestring))
...
def challenge6():
    ...
    unknown_bytes = find_appended_prefix(
        cipher.encrypt, prefix_length, block_size)
