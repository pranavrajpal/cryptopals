...
def sha1_length_extension(cipher, key_length):
    ...
    final_block_length = message_len_bits + \
        (len(glue_padding) + len(added_message)) * 8
    new_mac = sha1_hash(added_message, registers=registers,
                        message_length_bits=final_block_length)
