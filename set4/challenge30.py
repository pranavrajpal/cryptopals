...
def md4_hash(message, input_message_length_bits=None, registers=None):
    ...
    for bytestring in blocks:
        ...
        a, b, c, d = md5_round(a, b, c, d, k_values_round1,
                               s_values_round1, round1_brackets, block)
        # Round #2

        k_values_round2 = [0, 4, 8, 12, 1, 5,
                           9, 13, 2, 6, 10, 14, 3, 7, 11, 15]
        s_values_round2 = [3, 5, 9, 13]
        a, b, c, d = md5_round(a, b, c, d, k_values_round2,
                               s_values_round2, round2_brackets, block)

        # Round #3

        k_values_round3 = [0, 8, 4, 12, 2, 10,
                           6, 14, 1, 9, 5, 13, 3, 11, 7, 15]
        s_values_round3 = [3, 9, 11, 15]
        a, b, c, d = md5_round(a, b, c, d, k_values_round3,
                               s_values_round3, round3_brackets, block)
        a += np.uint32(saved_a)
        b += np.uint32(saved_b)
        c += np.uint32(saved_c)
        d += np.uint32(saved_d)
...
def test_md4_hash():
    tests = {"": '31d6cfe0d16ae931b73c59d7e0c089c0',
...
def md4_length_extension(cipher):
    ...
    new_length_bits = (original_length_bytes +
                       len(glue_padding) + len(to_append)) * 8
    new_mac = md4_hash(
